...
def editar_imagem(request, foto_id):
    ...
    # OWASP A01: Broken Access Control (Prevenção de IDOR - apenas autor ou admin pode editar)
    if fotografia.usuario != request.user and not request.user.is_staff:
        messages.error(request, 'Acesso negado: Você não tem permissão para editar esta fotografia.')
        return redirect('index')
...
def deletar_imagem(request, foto_id):
    ...
    # OWASP A01: Broken Access Control (Prevenção de IDOR - apenas autor ou admin pode excluir)
    if fotografia.usuario != request.user and not request.user.is_staff:
        messages.error(request, 'Acesso negado: Você não tem permissão para excluir esta fotografia.')
        return redirect('index')
